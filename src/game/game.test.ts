...
describe('Versetzen (#12, Constraints)', () => {
  it('versetzt unbedeckte eigene Kugel eine Ebene hoch, Reserve unverändert', () => {
    ...
    expect(moves.some(m =>
      ...
      m.targets.some(t => t.level === 0 && t.x === 2 && t.y === 2)
    )).toBe(true)
    expect(executeMoveExisting(s, p(0, 2, 2), p(1, 0, 0))).toBe(true)
    expect(getBall(s.board, p(0, 2, 2))).toBeNull()
    expect(getBall(s.board, p(1, 0, 0))?.color).toBe('light')
    expect(s.players[0].reserve).toBe(reserveBefore)
  })

  it('verbietet Versetzen aus dem stützenden Quadrat des Ziels (#360)', () => {
    const s = createInitialState()
    placeBall(s.board, p(0, 0, 0), 'light')
    placeBall(s.board, p(0, 1, 0), 'dark')
    placeBall(s.board, p(0, 0, 1), 'dark')
    ...
    expect(getBall(s.board, p(0, 0, 0))?.color).toBe('light')
    expect(getBall(s.board, p(1, 0, 0))).toBeNull()
    const moveExisting = getAvailableMoves(s).find(m => m.type === 'move_existing')
    expect(moveExisting).toBeDefined()
    expect(moveExisting?.targets.some(t => t.level === 0 && t.x === 0 && t.y === 0)).toBe(true)
  ...
  it('bedeckte Kugel wird nicht als versetzbar angeboten (#354)', () => {
    ...
    placeBall(s.board, p(0, 2, 3), 'dark')
    placeBall(s.board, p(0, 3, 3), 'light')
    placeBall(s.board, p(0, 0, 3), 'light')
    const moveExisting = getAvailableMoves(s).find(m => m.type === 'move_existing')
    expect(moveExisting).toBeDefined()
    ...
    expect(moveExisting?.targets.some(t => t.level === 0 && t.x === 2 && t.y === 2)).toBe(false)
    expect(moveExisting?.targets.some(t => t.level === 0 && t.x === 3 && t.y === 3)).toBe(false)
    expect(moveExisting?.targets.some(t => t.level === 0 && t.x === 0 && t.y === 3)).toBe(true)
  })
})
